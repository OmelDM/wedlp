	...
	$.fn.ajaxrsvp = function(options) {
  	...
  	var settings = $.extend( {},defaults, options );
		...
		this.submit(function(event){
				...
				else
				{
					...
					if ( this_input_value == null  || this_input_value.length == 0 ) {
						if ($(this).attr("data-required"))
						{
							...
							// add .has-error to input (text, textarrea, select options)
            	$(this).addClass("has-error");
							all_err.push(element.getAttribute("data-required"));
						}
       		 		}
			...
			var submit_sending_value = $('input[type="submit"]#submitButton').attr("data-sending-label");
			$('input[type="submit"]#submitButton').val(submit_sending_value);
